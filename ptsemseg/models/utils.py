...
class conv2DBatchNorm(nn.Module):
    def __init__(self, in_channels, n_filters, k_size,  stride, padding, bias=True, dilation=1, with_bn=True, use_gn=False):
        ...
        if use_gn:
            if with_bn:
                self.cb_unit = nn.Sequential(conv_mod,
                                             bn_func(int(n_filters)),
                                             torch.nn.GroupNorm(num_groups=32, num_channels=int(n_filters)),
                                             nn.Dropout2d(p=0.01, inplace=False)) 
            else:
                self.cb_unit = nn.Sequential(conv_mod,
                                             torch.nn.GroupNorm(num_groups=32, num_channels=int(n_filters)),
                                             nn.Dropout2d(p=0.01, inplace=False)) 
...
class conv2DBatchNormRelu(nn.Module):
        ...
        if use_gn:
            if with_bn:
                self.cbr_unit = nn.Sequential(conv_mod,
                                             bn_func(int(n_filters)),
                                             torch.nn.GroupNorm(num_groups=32, num_channels=int(n_filters)),
                                             nn.Dropout2d(p=0.05, inplace=False),nn.ReLU(inplace=True),) 
            else:
                self.cbr_unit = nn.Sequential(conv_mod,nn.ReLU(inplace=True),
                                             torch.nn.GroupNorm(num_groups=32, num_channels=int(n_filters)),
                                             nn.Dropout2d(p=0.05, inplace=False)) 
        else:
            if with_bn:
                self.cbr_unit = nn.Sequential(conv_mod,
                                              bn_func(int(n_filters)), 
    ...
    def forward(self, inputs, stop=False):
...
class pyramidPooling(nn.Module):
    ...
    def __init__(self, in_channels, pool_sizes, model_name='pspnet', fusion_mode='cat', with_bn=True,dim=-1, use_gn=False):
        ...
        for i in range(len(pool_sizes)):
            if dim < 0:
                self.paths.append(conv2DBatchNormRelu(in_channels, int(in_channels / len(pool_sizes)), 1, 1, 0, bias=bias, with_bn=with_bn, use_gn=use_gn))
            else:
                self.paths.append(conv2DBatchNormRelu(in_channels, dim, 1, 1, 0, bias=bias, with_bn=with_bn, use_gn=use_gn))
    ...
    def forward(self, x):
        ...
        if self.training or self.model_name != 'icnet': # general settings or pspnet
            ...
            for pool_size in self.pool_sizes:
                ...
                '''
                k_sizes.append((int(pool_size), int(pool_size)))
                strides.append((int(pool_size), int(pool_size)))
                #'''
...
class bottleNeckPSP(nn.Module):
    ...
    def __init__(self, in_channels, mid_channels, out_channels, 
                 stride, dilation=1, with_bn=True, relu=True):
...
class bottleNeckIdentifyPSP(nn.Module):
    ...
    def __init__(self, in_channels, mid_channels, stride, dilation=1, with_bn=True, relu=True):
...
class residualBlockPSP(nn.Module):
    ...
    def __init__(self, n_blocks, in_channels, mid_channels, out_channels, stride, dilation=1, include_range='all', with_bn=True, relu=True):
        ...
        if include_range in ['all', 'identity']:
            for i in range(n_blocks-1):
                #use_relu = (not relu) and (i == n_blocks - 2)
                if i == n_blocks - 2 and not relu:
                    layers.append(bottleNeckIdentifyPSP(out_channels, mid_channels, stride, dilation, with_bn=with_bn, relu=False))
                else:
